...
def search_anilist(title: str) -> dict | None:
    """AniList GraphQL API로 애니 검색 (시리즈 캐싱)"""
    ...
    try:
        ...
        r = requests.post(ANILIST_URL,
                          json={'query': ANILIST_QUERY, 'variables': {'search': normalized}},
                          timeout=5)
        ...
        # smry
        smry = media.get('description') or None
        if smry:
            smry = re.sub(r'<[^>]+>', '', smry).replace('\n', ' ').strip()
            if len(smry) < 10:
                smry = None
        ...
        if sd.get('year'):
            month = str(sd.get('month', 1) or 1).zfill(2)
            day = str(sd.get('day', 1) or 1).zfill(2)
            release_date = f"{sd['year']}-{month}-{day}"
        else:
            release_date = None

        # 제작사
        nodes = (media.get('studios') or {}).get('nodes', [])
        asset_prod = nodes[0]['name'] if nodes else None
...
def update_vod(conn, full_asset_id: str, data: dict) -> bool:
    fields, values = [], []
    for col in ('smry', 'genre', 'release_date', 'asset_prod'):
        if data.get(col):
...
def process_vod(vod: dict) -> tuple[str, bool]:
    ...
    conn = pool.getconn()
    try:
        result = search_anilist(vod['asset_nm'])
...
def run(max_workers: int = 3):
    print('=' * 50)
    print('AniList — TV애니메이션 not_found 처리')
    print('=' * 50)
